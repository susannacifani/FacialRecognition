...
def reports_to_txt(file_name, toScale=False, resampled=False, resampleTechnique=0):
    ...
    for train_index, test_index in kf.split(X,y):
        ...
        knn_grid = GridSearchCV(knn, knn_params, cv=kf, n_jobs=-1, verbose=True)
        knn_grid.fit(X_train, y_train)
        y_true, y_pred = y_test, knn_grid.predict(X_test)
        ...
        print(knn_grid.best_params_, file=output_file)
